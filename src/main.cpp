...
Item createWarmog() {
  return Item{"Warmog's Armor", {{Stat::HP, ModType::Base, 1000.0, "Warmog"}}, {}, {}};
}

Item createRabadon() {
  return Item{"Rabadon's Deathcap",
...
Item createBotRK() {
  return Item{
    ...
    {},
    {CombatOverlay{"BotRK On-Hit (9% Current HP)",
                   "",
...
void initVladimir(Champion &champ) {
  ...
  champ.statPassives.push_back(
    {"Passive: Crimson Pact", [](const Champion &champ, ModDB &db) {
...
void initVayne(Champion &champ) {
  ...
  champ.combatPassives.push_back(
    CombatOverlay{"Vayne W - Silver Bolts (8% Max HP True Damage)",
                  "",
                  [](const Damage &, const Damage &, const Champion &, const Champion &def) {
                    Damage delta;
...
void printChampionStats(const Champion &champ) {
  ...
  for (auto stat : statList) {
    std::cout << "    " << std::left << std::setw(8) << statToString(stat) << ": "
              << champ.get(stat) << "\n";
  }
}
...
void printDamage(const Damage &dmg) {
  ...
  std::cout << "    Physical: " << dmg.getFinalPhysical() << " (base=" << dmg.physical
            << ", mult=" << dmg.physicalMult << "x)\n";
