...
public:
  ...
  [[nodiscard]] double sumExclude(Stat stat,
                                  ModType type,
                                  const std::string &exclude_source) const {
    double total = 0.0;
  ...
  [[nodiscard]] bool removeBySource(const std::string &source) {
    auto it = std::remove_if(
      mods_.begin(), mods_.end(), [&](const Modifier &m) { return m.source == source; });
    if (it == mods_.end()) {
      return false;
    }
...
struct CombatOverlay {
  ...
  std::function<Damage(const Damage &finalDmg,
                       const Damage &prevDmg,
                       const Champion &attacker,
...
inline double calculateSingleStat(const Champion &champ,
                                  Stat stat,
...
inline void updateChampionStats(Champion &champ) {
  static constexpr std::array statList{
    Stat::HP, Stat::AP, Stat::AD, Stat::MS, Stat::Armor, Stat::MR};
...
inline std::string itemSource(const Item &item) {
...
inline void unequipItem(Champion &champ, const Item &item) {
  ...
  auto spIt = std::remove_if(champ.statPassives.begin(),
                             champ.statPassives.end(),
                             [&](const StatPassiveEntry &e) { return e.source == source; });
  champ.statPassives.erase(spIt, champ.statPassives.end());

  auto cpIt = std::remove_if(champ.combatPassives.begin(),
                             champ.combatPassives.end(),
...
inline Damage simulateAttack(const Champion &attacker, const Champion &defender) {
  ...
  pipeline.push_back(CombatOverlay{
    .name   = "Base Auto-Attack (AD)",
    .source = "",
    .apply  = [](const Damage &, const Damage &, const Champion &att, const Champion &) {
      Damage delta;
  ...
  pipeline.push_back(CombatOverlay{
    ...
    .source = "",
    .apply  = [](const Damage &, const Damage &prev, const Champion &, const Champion &def) {
      Damage delta;
